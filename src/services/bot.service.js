...
export class TgBot {
    constructor(tg_token, exchanges) {
        ...
        this.exchangeManager = new CryptoExchangeManager();
    ...
    async _getDepositOrder(userInputValue) {
        const balances = await this.exchangeManager.getDeposits(this.currentExchange, MAX_CHECKING_DURATION);
    ...
    _findMatchingElement(array, targetAmount) {
        ...
        return array.find((item) => Number(item.amount) >= lowerBound && Number(item.amount) <= upperBound);
    }
